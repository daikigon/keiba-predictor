...
export default function ModelPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">モデル管理（ばんえい競馬）</h1>
        <p className="text-gray-500 mt-1">ばんえい競馬用の機械学習モデルの管理とシミュレーション</p>
      </div>
...
const SIMULATION_HISTORY_KEY = 'simulation_history_banei';
...
function SimulationSection({ backendAvailable }: { backendAvailable: boolean }) {
  ...
  const [simParams, setSimParams] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('simParams_banei');
      if (saved) {
        try {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('simParams_banei', JSON.stringify(simParams));
    }
  }, [simParams]);
...
const SWEEP_HISTORY_KEY = 'threshold_sweep_history_banei';
const MAX_HISTORY = 5;
